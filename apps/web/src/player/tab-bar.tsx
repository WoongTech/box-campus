...
function TabButton({
  ...
  return (
    <button
      ...
      className={`relative flex items-center justify-center ${current ? "text-white" : "text-white/35"}`}
      onClick={onClick}
    >
      <AppIcon name={name} size={TAB_ICON} strokeWidth={current ? 2.2 : 1.7} />
      {current ? (
        <span className="absolute bottom-1.5 size-1 rounded-full bg-white" aria-hidden />
      ) : null}
    </button>
  );
}
