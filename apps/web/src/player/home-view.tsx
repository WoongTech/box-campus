      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-[calc(3.5rem+env(safe-area-inset-bottom))]">
        {posts.length === 0 ? (
          <button type="button" className="w-full px-6 py-16 text-center" onClick={onCompose}>
            ...
            <span className="mt-1 block text-xs text-secondary">추가로 첫 주제를 만드세요.</span>
          </button>
        ) : (
          posts.map((post) => {
                ...
                {active && showResume ? (
                  <button type="button" className="w-full px-4 py-3 text-left active:bg-white/5" onClick={onOpenStory}>
                    <span className="flex items-baseline justify-between gap-3">
                      ...
                      <span className="shrink-0 text-xs text-secondary">이어서</span>
                    </span>
                    <span className="mt-1 line-clamp-3 block text-[15px] leading-snug">{hero}</span>
                  </button>
                ) : null}
                {post.weeks.map((week) => {
                  const heading =
                    ...
                    <div key={week.id}>
                      ...
                      {week.ideaIds.length === 0 ? (
                        ...
                        >
                          ...
                          <span className="mt-1 block text-xs text-secondary">{emptyHint}</span>
                        </button>
                      ) : (
                              ...
                              <span className="flex items-start gap-3">
                                ...
                                <span className="min-w-0 flex-1">
                                  <span className="flex items-baseline justify-between gap-3">
                                    ...
                                    {current ? <span className="shrink-0 text-xs text-secondary">이어서</span> : null}
                                  </span>
                                  {line ? (
                                    <span className="mt-1 line-clamp-2 block text-[15px] leading-relaxed text-primary/90">
                                      {line}
                                    </span>
                                  ) : null}
                                </span>
        ...
        {posts.some((post) => post.id === state.campus.id) ? (
          <button
            type="button"
            className="mb-6 w-full py-4 text-center text-xs text-secondary"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-[calc(3.5rem+env(safe-area-inset-bottom))]">
        {items.length === 0 ? (
          <div className="flex flex-col items-center px-8 py-20 text-center">
            ...
            <p className="mt-1 text-xs leading-relaxed text-secondary">스토리에서 표시해 두면 여기에 모입니다.</p>
          </div>
        ) : (
          items.map((item) => (
            <button
